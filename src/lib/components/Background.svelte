<script lang="ts">
	...
	const uniformFillAlpha = 0.075; // User's preferred alpha
	const uniformStrokeAlpha = 0.75; // Subtle border alpha
	const unHoveredAlpha = 0.25;
	...
	interface InteractiveGraphics extends Graphics {
		targetAlpha?: number;
	}

	let backgroundContentContainer: PixiContainer; // Pixi container for all hexagon Graphics objects
	const hexPool: InteractiveGraphics[] = []; // ++ OBJECT POOL for hexagon Graphics objects
	const hexPointCache: Record<number, number[]> = {}; // ++ CACHE for hexagon points

	function getHexagonPoints(radius: number, centerX: number = 0, centerY: number = 0): number[] {
		const cached = hexPointCache[radius];
		if (cached) {
			return cached;
		}
		const points: number[] = [];
		...
		hexPointCache[radius] = points;
		return points;
	}

	class HexagonWithHoleHitArea {
		outerPolygon: Polygon;
		innerPolygon?: Polygon;

		constructor(outerPolygon: Polygon, innerPolygon?: Polygon) {
			this.outerPolygon = outerPolygon;
			this.innerPolygon = innerPolygon;
		}

		contains(x: number, y: number): boolean {
			if (!this.outerPolygon.contains(x, y)) return false;
			if (this.innerPolygon && this.innerPolygon.contains(x, y)) return false;
			return true;
		}
	}
	...
	function createHexTile(
		...
		graphics
			...
			.fill({ color: primaryColorHex, alpha: uniformFillAlpha })
			.stroke({ color: primaryContainerColorHex, width: 1.5, alpha: uniformStrokeAlpha });

		let innerRadius = 0;
		if (hasHole && displayRadius > HOLE_SUITABILITY_THRESHOLD) {
			const calculatedInnerRadius = displayRadius * holeRadiusRatio;
			if (calculatedInnerRadius > ABSOLUTE_MIN_RADIUS * 0.25) {
				...
				graphics
					...
					.stroke({ color: primaryContainerColorHex, width: 1, alpha: uniformStrokeAlpha });
			}
		}

		if (displayRadius > 1) {
			const outerPoints = getHexagonPoints(displayRadius - 0.5);
			if (outerPoints.length >= 6) {
				const outerPoly = new Polygon(outerPoints);
				let innerPoly: Polygon | undefined = undefined;
				if (innerRadius > 0) {
					const innerPoints = getHexagonPoints(innerRadius - 0.5);
					if (innerPoints.length >= 6) {
						innerPoly = new Polygon(innerPoints);
					}
				}
				graphics.hitArea = new HexagonWithHoleHitArea(outerPoly, innerPoly);
	...
	function generateRandomHexagons(
		...
		for (const hexConfig of potentialHexagons) {
			...
			let tile: InteractiveGraphics;
			if (activeHexes < hexPool.length) {
				tile = hexPool[activeHexes]!;
			} else {
				tile = new Graphics() as InteractiveGraphics;
				tile.on('pointerover', () => {
					tile.targetAlpha = 1.0;
				});
				tile.on('pointerout', () => {
					tile.targetAlpha = unHoveredAlpha;
				});
			...
			tile.visible = true;
			tile.targetAlpha = unHoveredAlpha;
			tile.alpha = unHoveredAlpha; // Reset alpha to normal when recycling
			activeHexes++;
			// -- END POOLING LOGIC
	...
	onMount(async () => {
		if (hostElement) {
			...
			app.ticker.add((ticker) => {
				...
				const speed = 0.12;
				for (const tile of hexPool) {
					if (tile.visible && tile.targetAlpha !== undefined) {
						tile.alpha += (tile.targetAlpha - tile.alpha) * speed * delta;
						if (Math.abs(tile.alpha - tile.targetAlpha) < 0.001) {
...
<div bind:this={hostElement} class="background-parallax-container">
